...
import { GatewayProcurementModule } from './gateway_procurement.module'
...
async function bootstrap() {
  ...
  const options = new DocumentBuilder()
    .setTitle('HORAS')
    .setVersion('1')
    .addServer(configService.get<string>('gateway_procurement.host_port'))
    .addBearerAuth(
      {
        name: 'JWT Bearer',
  ...
  await app.listen(
    parseInt(configService.get<string>('gateway_procurement.port'))
  )
