...
import { GatewayQueueModule } from './gateway_queue.module'
...
async function bootstrap() {
  ...
  const options = new DocumentBuilder()
    .setTitle('HORAS')
    .setVersion('1')
    .addServer(configService.get<string>('gateway_queue.host_port'))
    .addBearerAuth(
      {
        name: 'JWT Bearer',
  ...
  await app.listen(parseInt(configService.get<string>('gateway_queue.port')))
