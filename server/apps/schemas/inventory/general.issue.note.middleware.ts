...
import { GeneralIssueNote, GeneralIssueNoteSchema } from './general.issue.note'

@Injectable()
@MongoSubscriber({
  name: GeneralIssueNote.name,
  schema: GeneralIssueNoteSchema,
})
export class MongoMiddlewareGeneralIssueNote {
  constructor(
  ...
  async beforeSave(message: any) {
    const time = new TimeManagement()
    if (message.isNew) {
      message.id = `general_issue_note-${message._id}`
      message.__v = 0
    }
