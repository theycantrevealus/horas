...
import { ConfigGroup, ConfigGroupSchema } from './config.group'

@Injectable()
@MongoSubscriber({ name: ConfigGroup.name, schema: ConfigGroupSchema })
export class MongoMiddlewareConfigGroup {
  constructor(
  ...
  async beforeSave(message: any) {
    const time = new TimeManagement()
    if (message.isNew) {
      message.id = `config_group-${message._id}`
      message.__v = 0
    }
