import { mockAccount } from '@gateway_core/account/mock/account.mock'
import { mockAuthority } from '@gateway_core/account/mock/authority.mock'
import {
  masterAssessmentFormDocArray,
  mockMasterAssessmentForm,
  mockMasterAssessmentFormModel,
} from '@gateway_core/master/mock/master.assessment.form.mock'
import { MasterAssessmentFormService } from '@gateway_core/master/services/master.assessment.form.service'
import { LogActivity } from '@log/schemas/log.activity'
import { LogLogin } from '@log/schemas/log.login'
import { CACHE_MANAGER } from '@nestjs/cache-manager'
import { HttpStatus } from '@nestjs/common'
import { ConfigService } from '@nestjs/config'
import { JwtService } from '@nestjs/jwt'
import { getModelToken } from '@nestjs/mongoose'
import { Test, TestingModule } from '@nestjs/testing'
import { Account } from '@schemas/account/account.model'
import { Authority } from '@schemas/account/authority.model'
import {
  MasterAssessmentForm,
  MasterAssessmentFormDocument,
} from '@schemas/master/master.assessment.form'
import { AuthService } from '@security/auth.service'
import { GlobalResponse } from '@utility/dto/response'
import { WINSTON_MODULE_PROVIDER } from '@utility/logger/constants'
import { modCodes } from '@utility/modules'
import { testCaption } from '@utility/string'
import { Model } from 'mongoose'

describe('Master Assessment Form Service', () => {
  let configService: ConfigService
  let masterAssessmentFormService: MasterAssessmentFormService
  let masterAssessmentFormModel: Model<MasterAssessmentForm>

  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      controllers: [],
      providers: [
        MasterAssessmentFormService,
        AuthService,
        JwtService,
        {
          provide: ConfigService,
          useValue: {
            get: () => jest.fn().mockResolvedValue('Test'),
            set: () => jest.fn().mockResolvedValue('Test'),
          },
        },
        {
          provide: CACHE_MANAGER,
          useValue: {
            get: () => 'any value',
            set: () => jest.fn(),
          },
        },
        {
          provide: WINSTON_MODULE_PROVIDER,
          useValue: {
            log: jest.fn(),
            warn: jest.fn(),
            verbose: jest.fn(),
            error: jest.fn(),
          },
        },
        {
          provide: getModelToken(MasterAssessmentForm.name, 'primary'),
          useValue: mockMasterAssessmentFormModel,
        },
        {
          provide: getModelToken(Account.name, 'primary'),
          useValue: {},
        },
        {
          provide: getModelToken(Authority.name, 'primary'),
          useValue: mockAuthority,
        },
        { provide: getModelToken(LogActivity.name, 'primary'), useValue: {} },
        { provide: getModelToken(LogLogin.name, 'primary'), useValue: {} },
      ],
    }).compile()

    configService = module.get<ConfigService>(ConfigService)

    masterAssessmentFormService = module.get<MasterAssessmentFormService>(
      MasterAssessmentFormService
    )
    masterAssessmentFormModel = module.get<Model<MasterAssessmentFormDocument>>(
      getModelToken(MasterAssessmentForm.name, 'primary')
    )

    jest.clearAllMocks()
  })

  afterEach(() => {
    jest.clearAllMocks()
  })

  it(
    testCaption('SERVICE STATE', 'component', 'Service should be defined'),
    () => {
      expect(masterAssessmentFormService).toBeDefined()
    }
  )

  describe(
    testCaption('GET DATA', 'data', 'Master Assessment Form - Fetch list'),
    () => {
      it(
        testCaption('HANDLING', 'data', 'Response validity', {
          tab: 1,
        }),
        async () => {
          jest.spyOn(masterAssessmentFormModel, 'aggregate').mockReturnValue({
            exec: jest.fn().mockReturnValue(masterAssessmentFormDocArray),
          } as any)
          await masterAssessmentFormService
            .all(
              `{
              "first": 0,
              "rows": 10,
              "sortField": "created_at",
              "sortOrder": 1,
              "filters": {}
            }`
            )
            .then((result: GlobalResponse) => {
              // Should classify transaction
              expect(result.transaction_classify).toEqual(
                'MASTER_ASSESSMENT_FORM_LIST'
              )

              // Not an empty string so be informative
              expect(result.message).not.toBe('')

              // Should return success code
              expect(result.statusCode.customCode).toEqual(
                modCodes.Global.success
              )

              // Should be an array of data
              expect(result.payload['data']).toBeInstanceOf(Array)

              // Data should be defined
              expect(result.payload['data']).toEqual(
                masterAssessmentFormDocArray
              )
            })
        }
      )

      it(
        testCaption('HANDLING', 'data', 'Response error on fetch data', {
          tab: 1,
        }),
        async () => {
          jest
            .spyOn(masterAssessmentFormModel, 'aggregate')
            .mockImplementation({
              exec: jest.fn().mockRejectedValue(new Error()),
            } as any)

          await expect(
            masterAssessmentFormService.all({
              first: 0,
              rows: 10,
              sortField: 'created_at',
              sortOrder: 1,
              filters: {},
            })
          ).rejects.toThrow(Error)
        }
      )
    }
  )

  describe(
    testCaption('GET DETAIL', 'data', 'Master Assessment Form - Fetch detail'),
    () => {
      it(
        testCaption('HANDLING', 'data', 'Response validity', {
          tab: 1,
        }),
        async () => {
          const findMock = masterAssessmentFormDocArray[0]
          masterAssessmentFormModel.findOne = jest
            .fn()
            .mockImplementationOnce(() => {
              return Promise.resolve(findMock)
            })

          await masterAssessmentFormService
            .detail(findMock.id)
            .then((result: GlobalResponse) => {
              // Deep equality check
              expect(result.payload).toEqual(findMock)

              // Should classify transaction
              expect(result.transaction_classify).toEqual(
                'MASTER_ASSESSMENT_FORM_GET'
              )

              // Not an empty string so be informative
              expect(result.message).not.toBe('')

              // Should return success code
              expect(result.statusCode.customCode).toEqual(
                modCodes.Global.success
              )
            })
        }
      )

      it(
        testCaption('HANDLING', 'data', 'Response error on get detail data', {
          tab: 1,
        }),
        async () => {
          const targetData = masterAssessmentFormDocArray[0]

          jest
            .spyOn(masterAssessmentFormModel, 'findOne')
            .mockImplementationOnce(() => {
              throw new Error()
            })

          await expect(async () => {
            await masterAssessmentFormService.detail(targetData.id)
          }).rejects.toThrow(Error)
        }
      )
    }
  )

  describe(
    testCaption('ADD DATA', 'data', 'Master Assessment Form - Add new data'),
    () => {
      it(
        testCaption(
          'DATA',
          'data',
          'Should add new master master assessment form'
        ),
        async () => {
          jest.spyOn(masterAssessmentFormModel, 'create')

          await masterAssessmentFormService
            .add(mockMasterAssessmentForm(), mockAccount())
            .then((result: GlobalResponse) => {
              // Should create id
              expect(result.payload).toHaveProperty('id')

              // Should classify transaction
              expect(result.transaction_classify).toEqual(
                'MASTER_ASSESSMENT_FORM_ADD'
              )

              // Not an empty string so be informative
              expect(result.message).not.toBe('')

              // Should return success code
              expect(result.statusCode.customCode).toEqual(
                modCodes.Global.success
              )
            })
        }
      )

      it(
        testCaption('DATA', 'data', 'Should replace code if not defined'),
        async () => {
          jest.spyOn(masterAssessmentFormService, 'add')
          const dataTest = mockMasterAssessmentForm()
          delete dataTest.document_version
          await masterAssessmentFormService
            .add(dataTest, mockAccount())
            .then((result: GlobalResponse) => {
              // Should create id
              expect(result.payload).toHaveProperty('id')

              // Should classify transaction
              expect(result.transaction_classify).toEqual(
                'MASTER_ASSESSMENT_FORM_ADD'
              )

              // Not an empty string so be informative
              expect(result.message).not.toBe('')

              // Should return success code
              expect(result.statusCode.customCode).toEqual(
                modCodes.Global.success
              )
            })
        }
      )

      it(
        testCaption('HANDLING', 'data', 'Response error on add data', {
          tab: 1,
        }),
        async () => {
          jest
            .spyOn(masterAssessmentFormModel, 'create')
            .mockImplementationOnce(() => {
              throw new Error()
            })

          await expect(async () => {
            await masterAssessmentFormService.add(
              mockMasterAssessmentForm(),
              mockAccount()
            )
          }).rejects.toThrow(Error)
        }
      )
    }
  )

  describe(
    testCaption('EDIT DATA', 'data', 'Master Assessment Form - Edit data'),
    () => {
      it(
        testCaption(
          'HANDLING',
          'data',
          'Should edit master master assessment form',
          {
            tab: 1,
          }
        ),
        async () => {
          jest.spyOn(masterAssessmentFormModel, 'findOneAndUpdate')

          await masterAssessmentFormService
            .edit(
              {
                ...mockMasterAssessmentForm(),
                __v: 0,
              },
              masterAssessmentFormDocArray[0].id
            )
            .then((result: GlobalResponse) => {
              // Should create id
              expect(result.payload).toHaveProperty('id')

              // Should classify transaction
              expect(result.transaction_classify).toEqual(
                'MASTER_ASSESSMENT_FORM_EDIT'
              )

              // Not an empty string so be informative
              expect(result.message).not.toBe('')

              // Should return success code
              expect(result.statusCode.customCode).toEqual(
                modCodes.Global.success
              )
            })
        }
      )

      it(
        testCaption('HANDLING', 'data', 'Response error if data is not found', {
          tab: 1,
        }),
        async () => {
          jest
            .spyOn(masterAssessmentFormModel, 'findOneAndUpdate')
            .mockReturnValue(null)

          await expect(async () => {
            await masterAssessmentFormService.edit(
              {
                ...mockMasterAssessmentForm(),
                __v: 0,
              },
              masterAssessmentFormDocArray[0].id
            )
          }).rejects.toThrow(Error)
        }
      )

      it(
        testCaption('HANDLING', 'data', 'Response error on edit data', {
          tab: 1,
        }),
        async () => {
          const targetID = mockMasterAssessmentForm().id
          jest
            .spyOn(masterAssessmentFormModel, 'findOneAndUpdate')
            .mockImplementationOnce(() => {
              throw new Error()
            })

          await expect(async () => {
            await masterAssessmentFormService.edit(
              {
                ...mockMasterAssessmentForm(),
                __v: 0,
              },
              targetID
            )
          }).rejects.toThrow(Error)
        }
      )
    }
  )

  describe(
    testCaption(
      'DELETE DATA',
      'data',
      'Master master assessment form - Delete data'
    ),
    () => {
      it(
        testCaption(
          'HANDLING',
          'data',
          'Should delete master master assessment form',
          {
            tab: 1,
          }
        ),
        async () => {
          jest.spyOn(configService, 'get').mockReturnValue('Asia/Jakarta')

          jest
            .spyOn(masterAssessmentFormModel, 'findOneAndUpdate')
            .mockResolvedValue({
              statusCode: {
                defaultCode: HttpStatus.OK,
                customCode: modCodes.Global.success,
                classCode: '',
              },
              message: 'Delete success message',
              payload: {},
              transaction_classify: 'MASTER_ASSESSMENT_FORM_DELETE',
              transaction_id: '',
            } satisfies GlobalResponse)

          await masterAssessmentFormService
            .delete(masterAssessmentFormDocArray[0].id)
            .then((result: GlobalResponse) => {
              // Should classify transaction
              expect(result.transaction_classify).toEqual(
                'MASTER_ASSESSMENT_FORM_DELETE'
              )

              // Not an empty string so be informative
              expect(result.message).not.toBe('')

              // Should return success code
              expect(result.statusCode.customCode).toEqual(
                modCodes.Global.success
              )
            })
        }
      )

      it(
        testCaption('HANDLING', 'data', 'Response error on delete data', {
          tab: 1,
        }),
        async () => {
          const targetID = mockMasterAssessmentForm().id
          jest
            .spyOn(masterAssessmentFormModel, 'findOneAndUpdate')
            .mockImplementationOnce(() => {
              throw new Error()
            })

          await expect(async () => {
            await masterAssessmentFormService.delete(targetID)
          }).rejects.toThrow(Error)
        }
      )
    }
  )

  afterAll(async () => {
    jest.clearAllMocks()
  })
})
