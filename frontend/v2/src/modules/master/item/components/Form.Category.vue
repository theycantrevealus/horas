      ...
      <Card>
        <template #content>
          <div class="grid p-fluid">
            <div class="col-3">
              <div class="field">
                <label for="itemFormCode">{{
                  $t('master.item.category.label.code.caption')
                }}</label>
                <InputText
                  id="itemFormCode"
                  v-model.trim="v$.code.$model"
                  :disabled="!allowDispatch('btnMasterItemCategoryEdit')"
                  :placeholder="$t('master.item.category.label.code.placeholder')"
                  type="text"
                />
            ...
            <div class="col-9">
              <div class="field">
                <label for="itemFormName">{{
                  $t('master.item.category.label.name.caption')
                }}</label>
                <InputText
                  id="itemFormName"
                  v-model.trim="v$.name.$model"
                  :disabled="!allowDispatch('btnMasterItemCategoryEdit')"
                  :placeholder="$t('master.item.category.label.name.placeholder')"
                  type="text"
                />
            ...
            <div class="col-12">
              <div class="field">
                <label for="itemFormRemark">{{
                  $t('master.item.category.label.remark.caption')
                }}</label>
                <Editor
        ...
        <template #footer>
          <div class="flex flex-row-reverse p-6">
            <Button
              label="Save"
              type="submit"
              :disabled="v$.$invalid && !allowDispatch('btnMasterItemCategoryEdit')"
              icon="pi pi-save"
...
import { storeMasterItemCategory } from '@/modules/master/item/store'
import type {
  MasterItemCategoryAddParameter,
  MasterItemCategoryEditParameter,
} from '@/modules/master/item/category/interfaces'
...
export default defineComponent({
  ...
  async created() {
    if (this.dialogRef?.data.mode === 'edit') {
      this.submitPermission = this.allowDispatch('btnMasterItemCategoryEdit')
      ...
      this.submitPermission = this.allowDispatch('btnMasterItemCategoryAdd')
    }
    this.rendered = true
  },
  async mounted() {
  ...
  methods: {
    ...
    async submitData(event: MouseEvent) {
      ...
      confirmation.require({
        ...
        accept: async () => {
          if (this.dialogRef?.data.mode === 'edit') {
            const parameter: MasterItemCategoryEditParameter = {
              code: this.code,
              name: this.name,
              remark: this.remark,
              __v: this.v ?? 0,
            ...
            const parameter: MasterItemCategoryAddParameter = {
              code: this.code,
              name: this.name,
              remark: this.remark,
            }
            await this.masterItemCategoryStore.add(parameter).then((response) => {
              this.closeDialog(response)
            })
          }
        },
