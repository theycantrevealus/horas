      ...
      <Card>
        <template #content>
          <div class="grid p-fluid">
            <div class="col-3">
              <div class="field">
                <label for="itemFormCode">{{ $t('master.item.unit.label.code.caption') }}</label>
                <InputText
                  id="itemFormCode"
                  v-model.trim="v$.code.$model"
                  :disabled="!allowDispatch('btnMasterItemUnitEdit')"
                  :placeholder="$t('master.item.unit.label.code.placeholder')"
                  type="text"
                />
            ...
            <div class="col-9">
              <div class="field">
                <label for="itemFormName">{{ $t('master.item.unit.label.name.caption') }}</label>
                <InputText
                  id="itemFormName"
                  v-model.trim="v$.name.$model"
                  :disabled="!allowDispatch('btnMasterItemUnitEdit')"
                  :placeholder="$t('master.item.unit.label.name.placeholder')"
                  type="text"
                />
            ...
            <div class="col-12">
              <div class="field">
                <label for="itemFormRemark">{{
                  $t('master.item.unit.label.remark.caption')
                }}</label>
                <Editor
        ...
        <template #footer>
          <div class="flex flex-row-reverse p-6">
            <Button
              label="Save"
              type="submit"
              :disabled="v$.$invalid && !allowDispatch('btnMasterItemUnitEdit')"
              icon="pi pi-save"
...
import { storeMasterItemUnit } from '@/modules/master/item/store'
import type { MasterItemUnitAdd, MasterItemUnitEdit } from '@/modules/master/item/interfaces'
...
export default defineComponent({
  ...
  async created() {
    if (this.dialogRef?.data.mode === 'edit') {
      this.submitPermission = this.allowDispatch('btnMasterItemUnitEdit')
      ...
      this.submitPermission = this.allowDispatch('btnMasterItemUnitAdd')
    }
    this.rendered = true
  },
  async mounted() {
  ...
  methods: {
    ...
    async submitData(event: MouseEvent) {
      ...
      confirmation.require({
        ...
        accept: async () => {
          if (this.dialogRef?.data.mode === 'edit') {
            const parameter: MasterItemUnitEdit = {
              code: this.code,
              name: this.name,
              remark: this.remark,
              __v: this.v ?? 0,
            ...
            const parameter: MasterItemUnitAdd = {
              code: this.code,
              name: this.name,
              remark: this.remark,
            }
            await this.masterItemUnitStore.add(parameter).then((response) => {
              this.closeDialog(response)
            })
          }
        },
