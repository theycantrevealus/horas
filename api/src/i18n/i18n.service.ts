...
export class I18nService {
  ...
  async fetch_caption (route: string, iso: string) {
    return await this.i18nRepo.createQueryBuilder('i18n').where('route = :route', { route }).andWhere('iso_code_3 = :iso', { iso }).getMany()
  }
  ...
  async iso_list () {
    let isoList = []
    const data = await this.i18nRepo.createQueryBuilder('i18n').distinctOn(['iso_code_3']).getMany()
    data.map((e) => {
  ...
  async add (data: I18nAddDTO) {
    let response = new I18nAddDTOResponse()
  ...
  async edit (data: I18nEditDTO, id: string) {
    let response = new I18nEditDTOResponse()
  ...
  async delete_soft (id: number) {
    let response = new I18nDeleteDTOResponse()
    const oldMeta = await this.detail(id)
    var i18nRes = this.i18nRepo.softDelete({ id })
    if (i18nRes) {
