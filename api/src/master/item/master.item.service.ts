...
import { MasterItemAddDTO, MasterItemAddDTOResponse } from "./dto/master.item.add.dto";
import { MasterItemAliasAddDTO, MasterItemAliasAddDTOResponse } from "./dto/master.item.alias.add.dto";
...
import { MasterItemAliasEditDTO, MasterItemAliasEditDTOResponse } from "./dto/master.item.alias.edit.dto";
import { MasterItemEditDTO, MasterItemEditDTOResponse } from "./dto/master.item.edit.dto";
...
export class MasterItemService {
  ...
  async detail (uid: string) {
    return await this.masterItemRepo.createQueryBuilder('master_item').where('master_item.uid = :uid', { uid }).getOne()
  }

  async add (data: MasterItemAddDTO) {
    let response = new MasterItemAddDTOResponse()
    const masterItemRes = this.masterItemRepo.save(data).then(async returning => {
  ...
  async edit (item: MasterItemEditDTO, uid: string) {
    ...
    const masterItemRes = this.masterItemRepo.update(uid, {
      code: item.code,
      name: item.name,
      remark: item.remark
  ...
  async detail_alias (id: string) {
    return await this.masterItemAliasRepo.createQueryBuilder('master_x_item_alias').where('master_x_item_alias.id = :id', { id }).getOne()
  }

  async item_alias (uid: string) {
    return await this.masterItemAliasRepo.createQueryBuilder('master_x_item_alias').where('master_x_item_alias.item = :uid', { uid }).getMany()
  }

  async edit_alias (itemAlias: MasterItemAliasEditDTO, id: string) {
    let response = new MasterItemAliasEditDTOResponse()
    const masterItemAliasRes = this.masterItemAliasRepo.update(id, {
      name: itemAlias.name
    }).then(async returning => {
  ...
  async add_alias (data: MasterItemAliasAddDTO) {
    ...
    const masterItemAliasRes = this.masterItemAliasRepo.save(data).then(async returning => {
