      ...
      <template #header>
        <div>
          <Toolbar>
            <template #start>
              <Button
                v-if="credential.permission['btnAddLOV'] !== undefined"
                label="New"
                icon="pi pi-plus"
...
const LOVForm = defineAsyncComponent(() => import('@/modules/lov/components/Form.vue'))
export default {
  components: {
    DataTable,
    DynamicDialog,
  ...
  methods: {
    ...
    loadLazyData() {
      ...
      LOVService.getLOVList(this.lazyParams).then((response) => {
        if (response) {
